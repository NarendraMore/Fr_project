...
const Visiter = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = `${import.meta.env.VITE_APP_API_URL}/getVisitorReport`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = `${import.meta.env.VITE_APP_API_URL}/getVisitorRegistration`;
        const response = await fetch(apiUrl);
        if (!response.ok) {
          throw new Error("Network was not ok");
        }
  ...
  const deleteProduct = async () => {
    try {
      ...
      const apiUrl = `${import.meta.env.VITE_APP_API_URL}/deleteVisitorRegistration/${id}`;
